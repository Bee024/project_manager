    ...
    function runMainTickBody() {
      ...
      if (!assessmentDone && assessmentRoot && assessment.isAssessmentStarted(clickedButtons)) {
        ...
        if (qContainer) {
          ...
          if (!entry) {
            // --- OpenAI fallback ---
            if (!qContainer.dataset.aiPending && !qContainer.dataset.aiAnswered) {
              const apiKey = window.__OPENAI_API_KEY || '';
              if (!apiKey) {
                // No API key — wait for user
                if (!qContainer.dataset.warned) qContainer.dataset.warned = "true";
                const userDone = isQuestionAnswered(qContainer) || isSubmitEnabled(doc);
                if (!userDone) { scheduleAssessmentPoll(); return; }
                delete qContainer.dataset.warned;
                if (trySubmitAndAdvance(doc, qContainer)) { setStatus("Submit"); bumpCooldown(); invalidateShadowCache(); }
                scheduleAssessmentPoll();
                return;
              }
              qContainer.dataset.aiPending = "true";
              const options = deepQuerySelectorAll(".mcq__item, .js-mcq-item", qContainer);
              const choices = options.map((opt) => {
                const textEl = deepQuerySelectorAll(".mcq__item-text-inner", opt)[0];
                return textEl ? extractMcqOptionText(textEl) : "";
              }).filter(Boolean);
              const choiceList = choices.map((c, i) => `${i + 1}. ${c}`).join("\n");
              const messages = [
                { role: "system", content: "You are a CCNA exam assistant. Answer the multiple-choice question by returning ONLY the exact text of the correct answer option(s), one per line. No explanations." },
                { role: "user", content: choices.length
              ...
              fetch("https://api.openai.com/v1/chat/completions", {
                ...
                body: JSON.stringify({ model: "gpt-4o-mini", messages, max_tokens: 256, temperature: 0 })
              }).then((r) => r.json()).then((data) => {
                delete qContainer.dataset.aiPending;
                const answer = data?.choices?.[0]?.message?.content?.trim() || "";
                if (!answer) { qContainer.dataset.aiAnswered = "miss"; return; }
                const aiAnswers = answer.split("\n").map((l) => l.trim()).filter(Boolean);
                qContainer.dataset.aiAnswered = "true";
                const acted = selectMcqOptions(qContainer, aiAnswers, qText, { multiClick: true });
                if (acted) {
                  setStatus("AI");
                  bumpCooldown();
                  if (isQuestionAnswered(qContainer)) trySubmitAndAdvance(doc, qContainer);
                }
              }).catch(() => {
                delete qContainer.dataset.aiPending;
                qContainer.dataset.aiAnswered = "miss";
              });
              setStatus("AI...");
              scheduleAssessmentPoll();
              return;
            }
            if (qContainer.dataset.aiPending) {
              setStatus("AI...");
              scheduleAssessmentPoll();
              return;
            }
            // aiAnswered = "true" means already applied; aiAnswered = "miss" means AI failed
            if (!qContainer.dataset.warned) qContainer.dataset.warned = "true";
            const userDone = isQuestionAnswered(qContainer) || isSubmitEnabled(doc);
            if (!userDone) { scheduleAssessmentPoll(); return; }
            delete qContainer.dataset.warned;
